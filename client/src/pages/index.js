export { default as HomePage } from './home/HomePage';
export { default as AboutPage } from './AboutPage';
export { default as CreditPage } from './CreditPage';
export { default as FeedbackPage } from './FeedbackPage';
export { default as ImprintPage } from './ImprintPage';

// admin
export { default as AdminPage } from './admin/AdminPage';

// events
export { default as EventsPage } from './events/EventsPage';
export { default as EventDetailsPage } from './events/EventDetailsPage';

// places
export { default as PlaceDetailsPage } from './places/PlaceDetailsPage';
export { default as CulturePage } from './places/CulturePage';
export { default as CultureDetailsPage } from './places/CultureDetailsPage';
export { default as EducationPage } from './places/EducationPage';
export { default as EducationDetailsPage } from './places/EducationDetailsPage';
export { default as HotelsPage } from './places/HotelsPage';
export { default as HotelDetailsPage } from './places/HotelDetailsPage';
export { default as RestaurantsPage } from './places/RestaurantsPage';
export { default as RestaurantDetailsPage } from './places/RestaurantDetailsPage';
